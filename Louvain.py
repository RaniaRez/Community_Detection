...
class Louvain:
    def __init__(self, G):
        ...
        self._vid_vertex = {}
        self.communities = []  # List to store communities formed after each iteration
        ...
        self.communities.append(self._cid_vertices.copy())
    ...
    def first_stage(self):
        ...
        print(f'Communities at iteration 0: {self.communities[-1]}')
        self.lenli.append(len(self.communities[-1]))
        ...
        while True:
            ...
            for v_vid in visit_sequence:
                ...
                if max_delta_Q > 0.0 and cid != v_cid:
                    ...
                    mod_inc = True
                    self.communities.append(self._cid_vertices.copy())
                    self.iterat=self.iterat+1
                    print(f'Communities at iteration {len(self.communities)-1}: {self.communities[-1]}')
                    self.lenli.append(len(self.communities[-1]))
...
if __name__ == '__main__':
    ...
    G = load_graph('data/network.txt')
    obj = Graph()
    G1 = obj.createGraph("Data//network.txt")
    nx_G = nx.Graph(G)
    # Draw the graph using Matplotlib
    pos = nx.spring_layout(nx_G)
    nx.draw(nx_G, pos=pos, with_labels=True, font_color="white")
    plt.show()
    ...
    # Initialize an empty list to store the number of communities at each iteration
    num_communities = []
    import numpy as np
    # Loop through the communities and count the number of communities in each iteration
    num_communities = []
    num_communities = []
    for i, iteration in enumerate(communities):
        community_counts = Counter(iteration)
        print(community_counts)
        num_communities.append(np.ceil(len(community_counts)))
        print(num_communities)
        num_communities_rounded = np.ceil(num_communities)
    # Create a bar plot of the number of communities at each iteration
    plt.bar(range(len(num_communities)), num_communities)
    plt.xticks(range(len(num_communities)))
    plt.yticks(range(int(np.max(num_communities_rounded))+1))
    plt.title("Number of nodes created  for each community in the karate dataset")
    plt.xlabel("communauté")
    plt.ylabel("Nombre de noeuds dans la communauté")
    plt.show()
    
    num_communities = len(algorithm.lenli)
    bar_width =0.03  # adjust the width of bars as needed
    plt.bar(range(num_communities), algorithm.lenli, width=bar_width)
    plt.bar(range(len(algorithm.lenli)), algorithm.lenli, width=0.8)
    plt.xticks(range(0, len(algorithm.lenli), 10))
    plt.title("Number of communities created at each iteration in the network dataset")
    plt.xlabel("iteration")
    plt.ylabel("Number of communities")
    plt.show()


    # Plot the number of communities generated at each iteration
    plt.plot(range(len(algorithm.lenli)), algorithm.lenli)
    plt.xlabel('Iteration')
    plt.ylabel('Number of Communities')
    plt.title('Number of Communities Generated at Each Iteration for the football dataset')
    plt.show()
    print("number of communities",len(communities))
    print(communities)
    print("nb iteration",algorithm.iterat)
    print(algorithm.lenli)

    print("la qualité du partitionnement du graphe en communautés est", cal_Q(communities, G1))
    print(f'le temps execution de Louvain classique pour le cas karate  est de {end_time - start_time} secondes')
        # 可视化结果
